...
export class SocialService {
  ...
  async createThread(
    userId: number,
    data: { noiDung: string; hinhAnh?: string; threadChaId?: number },
  ) {
    ...
    if (
      data.threadChaId &&
      thread.threadCha?.tacGiaId &&
      thread.threadChaId !== userId
    ) {
  ...
  async getUserSocialProfile(userId: number, requesterId?: number) {
    ...
    let friendshipStatus: 'NONE' | 'FRIEND' | 'SENT' | 'RECEIVED' | 'BLOCKED' =
      'NONE';
    ...
    if (requesterId && requesterId !== userId) {
      ...
      if (friendship) {
        ...
        else if (friendship.trangThai === 'BI_CHAN')
          friendshipStatus = 'BLOCKED';
        else if (friendship.nguoiGuiId === requesterId)
    ...
    return {
      ...
      soDienThoai:
        (user as any).hoSoHocSinh?.soDienThoai ||
        (user as any).hoSoGiaoVien?.soDienThoai,
      gioiTinh:
        (user as any).hoSoHocSinh?.gioiTinh ||
        (user as any).hoSoGiaoVien?.gioiTinh,
      ngaySinh:
        (user as any).hoSoHocSinh?.ngaySinh ||
        (user as any).hoSoGiaoVien?.ngaySinh,
      diaChi:
        (user as any).hoSoHocSinh?.diaChiThuongTru ||
        (user as any).hoSoGiaoVien?.diaChi,
      isFollowing,
      friendshipStatus,
    };
  }

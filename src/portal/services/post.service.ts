...
export class PostService {
    ...
    async findAll(activeOnly: boolean = false, type?: LoaiBaiViet) {
        ...
        return this.prisma.baiViet.findMany({
            ...
            include: {
                nguoiTao: {
                    select: { id: true, taiKhoan: true, hoSoGiaoVien: { select: { hoTen: true } } }
                }
            }
        });
    }
    ...
    async findOne(slugOrId: string) {
        ...
        const post = await this.prisma.baiViet.findFirst({ // Changed to findFirst to allow OR logic if needed, but here simple findUnique/findFirst
            where: whereClause,
            include: {
                nguoiTao: {
                    select: { id: true, taiKhoan: true, hoSoGiaoVien: { select: { hoTen: true } } }

...
export class ChatService {
  ...
  async createChannel(userId: number, createChannelDto: CreateChannelDto) {
    if (createChannelDto.loaiKenh === LoaiKenhChat.CA_NHAN) {
      const friendId = createChannelDto.thanhVienIds?.find(
        (id) => id !== userId,
      );
      if (!friendId) {
    ...
    if (createChannelDto.thanhVienIds) {
      createChannelDto.thanhVienIds.forEach((id) => {
        if (id !== userId)
          members.push({ nguoiDungId: id, vaiTro: 'THANH_VIEN' }); // Others are members
      });
    }
  ...
  async getMessages(channelId: number, userId: number, page: number = 1) {
    ...
    const isMember = await this.prisma.thanhVienKenh.findUnique({
      where: {
        kenhChatId_nguoiDungId: { kenhChatId: channelId, nguoiDungId: userId },
      },
    });
    if (!isMember)
  ...
  async sendMessage(
    ...
    const isMember = await this.prisma.thanhVienKenh.findUnique({
      where: {
        kenhChatId_nguoiDungId: {
          kenhChatId: createMessageDto.kenhChatId,
          nguoiDungId: userId,
        },
      },
    });
    ...
    for (const memberId of otherMembers) {
      ...
      const notificationContent: string =
        message.loai === 'VAN_BAN'
          ? message.noiDung || ''
          : message.loai === 'HINH_ANH'
            ? '📷 Ảnh'
            : message.loai === 'GHI_AM'
              ? '🎤 Tin nhắn thoại'
              : '📎 Tệp đính kèm';

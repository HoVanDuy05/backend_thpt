...
export class FriendsService {
    ...
    const friendship = await this.prisma.ketBan.findFirst({
      where: {
        OR: [
          {
            nguoiGuiId: userId,
            nguoiNhanId: friendId,
            trangThai: TrangThaiBanBe.DA_KET_BAN,
          },
          {
            nguoiGuiId: friendId,
            nguoiNhanId: userId,
  ...
  async getFriends(userId: number) {
    ...
    return friendships.map((f: any) =>
      f.nguoiGuiId === userId ? f.nguoiNhan : f.nguoiGui,
    );
  }
  ...
  async checkStatus(userId: number, targetId: number) {
    ...
    if (friendship.trangThai === TrangThaiBanBe.DA_KET_BAN)
      return { status: 'FRIEND' };
    if (friendship.trangThai === TrangThaiBanBe.BI_CHAN)

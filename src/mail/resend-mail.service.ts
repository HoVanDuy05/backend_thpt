...
export class ResendMailService {
  ...
  private async renderTemplate(
    ...
    try {
      ...
      console.error(
        `[ResendMailService] Error rendering template ${templateName}:`,
        error,
      );
  ...
  async sendWelcomeEmail(user: any, locale: string = 'vi') {
    ...
    let subject =
      locale === 'vi'
        ? 'Chào mừng bạn đến với NHers Academy! 🎓'
        : 'Welcome to NHers Academy! 🎓';
    const title =
      locale === 'vi' ? 'Chào mừng thành viên mới' : 'Welcome new member';
    ...
    console.log(
      `[ResendMailService] Attempting to send Welcome Email to: ${toEmail} from: ${fromEmail}`,
    );

    if (fromEmail === 'onboarding@resend.dev') {
      if (toEmail !== OWNER_EMAIL) {
        console.warn(
          `[ResendMailService] SANDBOX MODE DETECTED. Redirecting email from ${toEmail} to ${OWNER_EMAIL}`,
        );
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send welcome email to ${toEmail}:`,
          JSON.stringify(error, null, 2),
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent welcome email to ${toEmail}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send welcome email (CRITICAL):`,
        error,
      );
    }
  }
  ...
  async sendVerificationEmail(user: any, code: string, locale: string = 'vi') {
    let subject =
      locale === 'vi'
        ? 'Mã xác thực tài khoản NHers Academy 🔐'
        : 'Verification Code - NHers Academy 🔐';
    const title =
      locale === 'vi' ? 'Xác thực bảo mật' : 'Security Verification';
    ...
    if (fromEmail === 'onboarding@resend.dev') {
      if (toEmail !== OWNER_EMAIL) {
        console.warn(
          `[ResendMailService] SANDBOX MODE DETECTED. Redirecting verification from ${toEmail} to ${OWNER_EMAIL}`,
        );
        subject = `[SANDBOX REDIRECT from ${toEmail}] ${subject}`;
        toEmail = OWNER_EMAIL;
      }
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send verification email to ${toEmail}:`,
          error,
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent verification email to ${toEmail}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send verification email (CRITICAL):`,
        error,
      );
    }
  }
  ...
  async sendResetPasswordEmail(
    ...
    const subject =
      locale === 'vi'
        ? 'Yêu cầu khôi phục mật khẩu NHers Academy 🔑'
        : 'Reset Password Request - NHers Academy 🔑';
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send reset email to ${user.email}:`,
          error,
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent reset email to ${user.email}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send reset email (CRITICAL):`,
        error,
      );
    }
  }
  ...
  async sendApprovalNotification(
    ...
    const subject =
      locale === 'vi'
        ? `Cập nhật phê duyệt: ${data.title} 📝`
        : `Approval Update: ${data.title} 📝`;
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send approval status email to ${to}:`,
          error,
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent approval status email to ${to}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send approval status email (CRITICAL):`,
        error,
      );
    }
  }
  ...
  async sendGradeNotification(submission: any, grade: any) {
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send grade notification to ${submission.hocSinh.email}:`,
          error,
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent grade notification to ${submission.hocSinh.email}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send grade notification (CRITICAL):`,
        error,
      );
    }
  }
  ...
  async sendAccountDetailsEmail(
    ...
    if (fromEmail === 'onboarding@resend.dev') {
      if (toEmail !== OWNER_EMAIL) {
        console.warn(
          `[ResendMailService] SANDBOX MODE DETECTED. Redirecting Account Credentials from ${toEmail} to ${OWNER_EMAIL}`,
        );
        subject = `[SANDBOX REDIRECT from ${toEmail}] ${subject}`;
        toEmail = OWNER_EMAIL;
      }
    ...
    try {
      ...
      if (error) {
        console.error(
          `[ResendMailService] FAILED to send account details email to ${toEmail}:`,
          JSON.stringify(error, null, 2),
        );
      } else {
        console.log(
          `[ResendMailService] Successfully sent account details email to ${toEmail}`,
          resData,
        );
      }
    } catch (error) {
      console.error(
        `[ResendMailService] FAILED to send account details email (CRITICAL):`,
        error,
      );
    }
  }
